...
//    private Set<Anchor> anchorPeers;
    ...
    public Channel initialize() throws InvalidArgumentException, TransactionException {
        ...
        try {
            ...
            registerChaincodeEventListenerProcessor();
            logger.debug(format("Channel %s registerChaincodeEventListenerProcessor completed", name));
...
//        }
    ...
    //////////  Chaincode events monitoring  /////////////////////////////

    static class ChaincodeEventProcessor {

        private final ChaincodeEventListener listener;
        private final String handle;
        private final Pattern chaincodeId;
        private final Pattern eventName;

        String getHandle() {
            return handle;
        }

        Pattern getChaincodeId() {
            return chaincodeId;
        }

        Pattern getEventName() {
            return eventName;
        }

        ChaincodeEventProcessor(Pattern chaincodeId, Pattern eventName, ChaincodeEventListener listener) {
            this.listener = listener;
            this.chaincodeId = chaincodeId;
            this.eventName = eventName;
            this.handle = Utils.generateUUID();
            logger.debug(format("Chaincode eventListener %s starting", handle));
        }
        ...
        public boolean equals(Object o) {
            ...
            ChaincodeEventProcessor that = (ChaincodeEventProcessor) o;
    ...
    private Map<String, ChaincodeEventProcessor> chaincodeEventListeners = new HashMap<>();

    /* Register an chaincode event listener
     * @param ccid chaincodeId
     * @param eventName event name to handle
     * @param chaincodeEventListener listener object
    ...
    {
        synchronized (chaincodeEventListeners) {
            final ChaincodeEventProcessor chaincodeEventListenerProcessor = new ChaincodeEventProcessor(chaincodeId, eventName, chaincodeEventListener);
            chaincodeEventListeners.put(chaincodeEventListenerProcessor.getHandle(), chaincodeEventListenerProcessor);
            return chaincodeEventListenerProcessor.getHandle();
        }
    }

    public void unregisterChaincodeEventListener(String handle)
    {
        synchronized (chaincodeEventListeners) {
            chaincodeEventListeners.remove(handle);
        }
    }

    /**
     * One block listener to manage chaincode events.
     *
     * @return
     */

    private String registerChaincodeEventListenerProcessor() throws InvalidArgumentException {
        logger.debug(format("Channel %s registerChaincodeEventListenerProcessor starting", name));

        // Transaction listener is internal Block listener for chaincode events

        return registerBlockListener(blockEvent -> {

                    synchronized (chaincodeEventListeners) {
                        if (chaincodeEventListeners.isEmpty()) {
                            return;
                        }
                    }

            for (TransactionEvent transactionEvent : blockEvent.getTransactionEvents()) {
                for (BlockInfo.TransactionEnvelopeInfo.TransactionActionInfo actionInfo : transactionEvent.getTransactionActionInfos()) {
                    String channelId = "<undefined>";
                    try {
                        channelId = blockEvent.getChannelId();
                    ...
                    class Pair
                    {
                        ...
                        final ChaincodeEventProcessor processor;

                        Pair(ChaincodeEvent chaincodeEvent, ChaincodeEventProcessor processor) {
                            this.processor = processor;
                            this.chaincodeEvent = chaincodeEvent;
                        }
                    }
                    ChaincodeEvent chaincodeEvent = actionInfo.getChaincodeEvent();
                    final List<Pair> matched = new ArrayList<>();

                    synchronized (chaincodeEventListeners) {
                        final String chaincodeId = chaincodeEvent.getChaincodeId();
                        final String eventName = chaincodeEvent.getEventName();
                        for (ChaincodeEventProcessor chaincodeEventListenerProcessor : chaincodeEventListeners.values()) {
                            if (chaincodeEventListenerProcessor.getChaincodeId().matcher(chaincodeId).matches()
                                    && chaincodeEventListenerProcessor.getEventName().matcher(eventName).matches()) {
                                matched.add(new Pair(chaincodeEvent, chaincodeEventListenerProcessor));
                            }
                    ...
                    for (Pair l : matched) {
                        try {
                            executorService.execute(() -> l.processor.listener.received(l.processor.getHandle(), blockEvent, chaincodeEvent));
                        } catch (Throwable e) { //Don't let one register stop rest.
                            logger.error(format("Error trying to call chaincode event listener on channel %s, chaincode %s, event %s", channelId, l.chaincodeEvent.getChaincodeId(), l.chaincodeEvent.getEventName()), e);
                        }
                    }
                }
            }
